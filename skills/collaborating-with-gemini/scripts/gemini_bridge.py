...
def main():
    ...
    parser.add_argument("--ais-profile", dest="ais_profile", default="", help="Use AnyAIToken (ais) to inject environment. Provide the profile name to run via 'ais run <profile> -- gemini ...'. If empty, runs gemini directly.")
    ...
    # Build the base gemini command
    gemini_cmd = ["gemini", "--prompt", PROMPT, "-o", "stream-json"]

    if args.sandbox:
        gemini_cmd.extend(["--sandbox"])

    if args.model:
        gemini_cmd.extend(["--model", args.model])

    if args.SESSION_ID:
        gemini_cmd.extend(["--resume", args.SESSION_ID])

    # Wrap with ais if --ais-profile is specified
    if args.ais_profile:
        cmd = ["ais", "run", args.ais_profile, "--"] + gemini_cmd
    else:
        cmd = gemini_cmd
