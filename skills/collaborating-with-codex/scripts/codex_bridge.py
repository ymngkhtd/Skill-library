...
def main():
    ...
    parser.add_argument("--ais-profile", dest="ais_profile", default="", help="Use AnyAIToken (ais) to inject environment. Provide the profile name to run via 'ais run <profile> -- codex ...'. If empty, runs codex directly.")

    args = parser.parse_args()

    # Build the base codex command
    codex_cmd = ["codex", "exec", "--sandbox", args.sandbox, "--cd", args.cd, "--json"]

    if args.image:
        codex_cmd.extend(["--image", ",".join(args.image)])

    if args.model:
        codex_cmd.extend(["--model", args.model])

    if args.profile:
        codex_cmd.extend(["--profile", args.profile])

    if args.yolo:
        codex_cmd.append("--yolo")

    if args.skip_git_repo_check:
        codex_cmd.append("--skip-git-repo-check")

    if args.SESSION_ID:
        codex_cmd.extend(["resume", args.SESSION_ID])
    ...
    # Wrap with ais if --ais-profile is specified
    if args.ais_profile:
        cmd = ["ais", "run", args.ais_profile, "--"] + codex_cmd
    else:
        cmd = codex_cmd
