...
usage: codex_bridge.py [-h] --PROMPT PROMPT --cd CD [--sandbox {read-only,workspace-write,danger-full-access}] [--SESSION_ID SESSION_ID] [--skip-git-repo-check]
                       [--return-all-messages] [--image IMAGE] [--model MODEL] [--yolo] [--profile PROFILE] [--ais-profile AIS_PROFILE]
...
options:
  ...
  --ais-profile AIS_PROFILE
                        Use AnyAIToken (ais) to inject environment when running the underlying `codex` command. Provide the profile name to run via
                        `ais run <profile> --`. If omitted, the bridge will call `codex` directly (backward compatible).
```
...
Always assume that AnyAIToken (`ais`) has already been used to manage API keys, you can inject environment variables via `ais run <profile> --`. The repository's `codex_bridge.py` supports the `--ais-profile <name>` parameter:

```bash
# Use the ais wrapper
ais run personal-codex -- python scripts/codex_bridge.py --cd "/path/to/project" --PROMPT "Your task"

# Or pass the profile directly to the script
python scripts/codex_bridge.py --cd "/path/to/project" --PROMPT "Your task" --ais-profile personal-codex
```

If `ais` is not installed, the script will fall back to calling `codex` directly, preserving backward compatibility.
